...
{
	//消息：数据总长度(2byte) + 数据类型(2byte) + 数据(N byte)
	public static int HEAD_DATA_LEN = 2;
	public static int HEAD_TYPE_LEN = 2;
	public static int HEAD_MSG_LEN = 2;
...
{//自动大小数据缓存器
	...
	public void UpdateDataLength()
	{
		if (_dataLength == 0 && _curBuffPosition >= Constants.HEAD_LEN)
		{
			...
			_buffLength = BitConverter.ToInt16(tmpDataLen, 0);
	...
	public bool GetData(out sSocketData _tmpSocketData)
	{
		...
		if (_buffLength > 0 && _curBuffPosition >= _buffLength)
		{
			...
			_tmpSocketData._data = new byte[_dataLength + 2];


			Array.Copy(_buff, Constants.HEAD_LEN, _tmpSocketData._data, 0, _dataLength + 2);
			_curBuffPosition -= _buffLength + 2;

			byte[] _tmpBuff = new byte[_curBuffPosition < _minBuffLen ? _minBuffLen : _curBuffPosition];
			Array.Copy(_buff, _buffLength + 2, _tmpBuff, 0, _curBuffPosition);
			_buff = _tmpBuff;
