...
public class SocketManager
{
    ...
    private void _onConnect_Sucess(IAsyncResult iar)
    {
        try
        {
            ...
            _isConnected = true;
			Debug.Log("连接成功");
			MessageCenter.Instance.PostEvent(eGameLogicEventType.ConnectServerSuccess);
    ...
    {
		byte[] _tmpBuff = new byte[tmpSocketData._buffLength + Constants.HEAD_MSG_LEN];
		byte[] _tmpBuffLength = BitConverter.GetBytes((UInt16)tmpSocketData._buffLength);
		byte[] _tmpProtoType = BitConverter.GetBytes((UInt16)tmpSocketData._protocallType);
		byte[] _tmpDataLen = BitConverter.GetBytes((UInt16)tmpSocketData._dataLength);

		Array.Reverse (_tmpBuffLength);
		Array.Reverse (_tmpProtoType);
		Array.Reverse (_tmpDataLen);

		Array.Copy(_tmpBuffLength, 0, _tmpBuff, 0, Constants.HEAD_DATA_LEN);//缓存总长度
		Array.Copy(_tmpProtoType, 0, _tmpBuff, Constants.HEAD_DATA_LEN, Constants.HEAD_TYPE_LEN);//协议类型
		Array.Copy(_tmpDataLen, 0, _tmpBuff, Constants.HEAD_DATA_LEN + Constants.HEAD_TYPE_LEN, Constants.HEAD_MSG_LEN);//协议类型
		Array.Copy(tmpSocketData._data, 0, _tmpBuff, Constants.HEAD_LEN + Constants.HEAD_MSG_LEN, tmpSocketData._dataLength);//协议数据
